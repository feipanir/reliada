...
def get_model_base(architecture, backbone):
    # if architecture == 'hrda1-512-0.1_daformer_sepaspp':
    #     pdb.set_trace()
    ...
    # if architecture == 'daformer_sepaspp':
    #     pdb.set_trace()
    return {
...
def generate_experiment_cfgs(id):
    ...
    # -------------------------------------------------------------------------
    ...
    elif id == 815: # an additional experiments without masking, in comparison to id==81
        # seeds = [0, 1, 2]
        seeds = [0]
        #        opt,     lr,      schedule,     pmult
        sgd   = ('sgd',   0.0025,  'poly10warm', False)
        ...
        for architecture,                      backbone,  uda_hp in [
            ...
            ('hrda1-512-0.1_dlv2red',          'r101v1c', uda_hrda),
            ('daformer_sepaspp',               'mitb5',   uda_daformer),
            ('hrda1-512-0.1_daformer_sepaspp', 'mitb5',   uda_hrda),
        ]:
            if 'hrda' in architecture:
                source, target, crop = 'gtaHR', 'cityscapesHR', '1024x1024'
                rcs_min_crop = 0.5 * (2 ** 2)
    ...
    elif id == 816: # an additional experiments without masking, in comparison to id==81
        # seeds = [0, 1, 2]
        seeds = [0]
        #        opt,     lr,      schedule,     pmult
        sgd   = ('sgd',   0.0025,  'poly10warm', False)
        adamw = ('adamw', 0.00006, 'poly10warm', True)
        #               uda,                  rcs_T, plcrop, opt_hp
        uda_advseg =   ('source-only',        None,  False,  *sgd)
        uda_minent =   ('source-only',        None,  False,  *sgd)
        uda_dacs =     ('source-only',        None,  False,  *adamw)
        uda_daformer = ('source-only',        0.01,  True,   *adamw)
        uda_hrda =     ('source-only',        0.01,  'v2',   *adamw)
        mask_mode, mask_ratio = None, 0
        for architecture,                      backbone,  uda_hp in [
            ('dlv2red',                        'r101v1c', uda_advseg),
            ('dlv2red',                        'r101v1c', uda_minent),
